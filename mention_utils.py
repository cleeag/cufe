...
def get_done_id():
    ...
    test_sample_mention_output_path = join(home_dir, 'data', 'amazon_test', f'{input_file}_amazon_{100}_test.csv')
    output_file_path = join(home_dir, 'data', 'amazon_test', 'done_mention_id.txt')
    mention_id_list = []
    with open(test_sample_mention_output_path, 'r') as r:
        reader = csv.reader(r)
        next(reader)
        for line in reader:
            mention_id_list.extend(line[:5])
    assert len(set(mention_id_list)) == 100
    with open(output_file_path, 'w') as w:
        w.writelines([x + '\n' for x in mention_id_list])


def get_test_from_amz(test_rng = (0, 100)):
    input_file = 'cufe_mention_sampled'
    amazon_input_path = join(home_dir, 'data', 'processed', f'{input_file}_amazon.csv')
    test_amazon_output_path = join(home_dir, 'data', 'amazon_test',
                                   f'{input_file}_amazon_{test_rng[0]}-{test_rng[1]}_test.csv')

    # done_mention_id_path = join(home_dir, 'data', 'amazon_test', 'done_mention_id.txt')
    # with open(done_mention_id_path, 'r') as r:
    #     done_set = set([x.strip() for x in r.readlines()])

    with open(amazon_input_path, 'r') as r, open(test_amazon_output_path, 'w') as tw:
        data = [line for line in csv.reader(r)]
        print(data[0])
        data = data[1:]
        test_data = data[test_rng[0] // 5: test_rng[1] // 5]
        print(len(test_data))
        csv.writer(tw).writerow(headers)
        csv.writer(tw).writerows(test_data)
...
def get_amz_csv_format():
    ...
    amazon_output_path = join(home_dir, 'data', 'processed', f'{input_file}_amazon.csv')

    with open(cufe_mention_path, 'r') as r, open(amazon_output_path, 'w') as w:
        data = r.readlines()
        random.shuffle(data)
        writer = csv.writer(w)
...
def preprocess_boson():
    ...
    with open(file_path, 'r') as r, open(output_file_path, 'w', encoding='utf-8') as w:
        ...
        for _ in tqdm(data):
            text = _
            # sentences_spl = re.split(r'([。！？])', text)
            sentences = []
            while re.search(r'[。！？?]', text):
                match_obj = re.search(r'[。！？?]', text)
                sentences.append(text[:match_obj.start() + 1])
                text = text[match_obj.start() + 1:]
            # assert len(sentences_spl) == len(sentences)
...
if __name__ == '__main__':
    ...
    # get_mention_file()
    # sample_mention_file()
    # get_amz_csv_format(test_num=1000)
    # get_done_id()
    get_test_from_amz(test_rng=(100, 1100))